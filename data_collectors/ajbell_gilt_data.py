        ...
        try:
            ...
            for i, row in enumerate(rows[1:]):  # Skip header
                try:
                    ...
                    # Debug: Log the actual bond_name_full format
                    self.logger.debug(f"Parsing bond_name_full: '{bond_name_full}'")
                    ...
                    self.logger.debug(f"Split parts: {parts} (count: {len(parts)})")
                    
                    if len(parts) >= 3:
                        short_code = parts[2].strip()  # Third part is the actual short code
                        self.logger.debug(f"Using 3rd part as short_code: '{short_code}'")
                    elif len(parts) >= 2:
                        ...
                        if re.match(r'^GB[A-Z0-9]{10}$', second_part):
                            ...
                            self.logger.debug(f"2nd part is ISIN, no short_code available: '{second_part}'")
                        else:
                            short_code = second_part  # Second part is the short code
                            self.logger.debug(f"Using 2nd part as short_code: '{short_code}'")
    ...
    try:
        ...
        for gilt in gilt_data:
            try:
                ...
                isin_value = str(gilt['isin']) if gilt['isin'] is not None else None
                short_code_value = str(gilt['short_code']) if gilt['short_code'] is not None else None
                
                # Debug log field lengths to help identify constraint violations
                if isin_value and len(isin_value) > 10:
                    collector.logger.debug(f"ISIN longer than 10 chars for {gilt['bond_name']}: '{isin_value}' ({len(isin_value)} chars)")
                if short_code_value and len(short_code_value) > 10:
                    collector.logger.warning(f"Short code longer than 10 chars for {gilt['bond_name']}: '{short_code_value}' ({len(short_code_value)} chars) - truncating to 10 chars")
